  ...
  try {
    ...
    if (existingUser) {
      ...
      if (serviceRoleKey) {
        ...
        console.log('⚠️  Cannot update password without SUPABASE_SERVICE_ROLE_KEY')
        console.log('   Please delete the user manually or add service role key to .env.local')
        process.exit(1)
      }
    } else {
      if (serviceRoleKey) {
